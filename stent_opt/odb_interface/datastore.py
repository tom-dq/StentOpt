...
class Datastore:
    ...
    def _DEBUG_CHECK_index_exists(self, nt_class):
        with self.connection:
            ...
            rows = list(
                self.connection.execute(query_str, (db_defs.make_index_name(nt_class),))
            )
            num = rows[0][0]
            if num == 0:
                return False
    ...
    def _add_results_with_frame_rowid(self, many_results, frame_rowid):
        with self.connection:
            ...
            for nt_class, iter_of_nts in itertools.groupby(many_results, type):
                ...
                with_row_id = (
                    nt._replace(frame_rowid=frame_rowid) for nt in iter_of_nts
                )
    ...
    def _get_rowid_of_frame_in_db(self, frame):
        with self.connection:
            rows = self.connection.execute(
                "SELECT rowid FROM Frame WHERE fn_odb = ? AND instance_name = ? AND step_num = ? AND frame_id = ?",
    ...
    def get_maybe_last_frame_of_instance(self, inst_name):
        with self.connection:
            rows = self.connection.execute(
                "SELECT * FROM Frame WHERE instance_name = ? ORDER BY rowid DESC LIMIT 1",
                (inst_name,),
            )
            for row in rows:
        ...
        logging.warning(
            "Did not get an instance_name of {0} in Frame table in {1}".format(
                inst_name, self.fn
            )
        )
    ...
    def get_all_rows_at_frame(
        self,
        named_tuple_class,
        frame,
        only_these_elem_nums=None,
        only_these_node_nums=None,
    ):
        with self.connection:
            select_string = "SELECT * FROM {0} WHERE frame_rowid=?".format(
                named_tuple_class.__name__
            )
            if only_these_elem_nums is not None:
                only_these_elem_nums_str = ",".join(
                    str(x) for x in only_these_elem_nums
                )
                select_string = select_string + " AND elem_num in ({0})".format(
                    only_these_elem_nums_str
                )

            if only_these_node_nums is not None:
                only_these_node_nums_str = ",".join(
                    str(x) for x in only_these_node_nums
                )
                select_string = select_string + " AND node_num in ({0})".format(
                    only_these_node_nums_str
                )
    ...
    def get_all_rows(self, named_tuple_class):
        with self.connection:
            select_string = "SELECT * FROM {0} ORDER BY frame_rowid".format(
                named_tuple_class.__name__
            )
    ...
    def get_final_history_result(self):
        with self.connection:
            rows = self.connection.execute(
                "SELECT * FROM HistoryResult ORDER BY history_identifier, step_num, simulation_time DESC"
            )
            ...
            for history_identifier, many_rows in itertools.groupby(
                rows_hr, get_history_identifier
            ):
...
if __name__ == "__main__":
    ...
    data = [
        ...
        db_defs.ElementStress(
            frame_rowid=None, elem_num=2, SP1=11.2, SP2=21.3, SP3=31.4
        ),
    ]
