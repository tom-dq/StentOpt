...
from stent_opt.struct_opt import (
    design,
    display,
    score,
    history,
...
class RunOneArgs(typing.NamedTuple):
    ...
    def fn_inp(self) -> pathlib.Path:
        return history.make_fn_in_dir(
            self.working_dir, ".inp", self.iter_this, self.patch_suffix
        )
        ...
        for patch_run_one_args in self.child_patch_run_one_args:
            for sub_model_info in patch_run_one_args.model_infos:
                ...
                for (
                    ...
                    sub_model_info.elem_nums
                ):  # this is always a patch_manager.SubModelInfo so elem_nums will be there.
                    attached_elems = [
                        elem_num
                        for elem_num in sub_model_info.elem_nums
                        if source_elem_num != elem_num
                    ]
    ...
    with history.History(history_db_fn) as hist:
        ...
        for iter_num in iteration_nums:
            ...
            with datastore.Datastore(output_db_fn) as data:
                ...
                raw_rows = list(
                    data.get_all_rows_at_frame(
                        optim_params.region_gradient.component, one_frame
                    )
                )
                raw_ranking_components = list(
                    score.get_primary_ranking_components(
                        True,
                        optim_params.single_primary_ranking_fitness_filter,
                        optim_params,
                        raw_rows,
                    )
                )
                element_ranking_components = {
                    one_comp.elem_id: one_comp for one_comp in raw_ranking_components
                }
                yield score.GradientInputData(
                    iteration_num=iter_num,
                    stent_design=design.make_design_from_snapshot(
                        stent_params, snapshot
                    ),
...
def gaussian_smooth(
    ...
    def normalised_sigma():
        ...
        elem_lengths = [
            stent_params.single_element_r_span,
            stent_params.single_element_theta_span,
            stent_params.single_element_z_span,
        ]
    ...
    if len(out_dict) < len(unsmoothed):
        logging.warning(
            f"Smoothing started with {len(out_dict)} but ended up with {len(unsmoothed)}. Sounds like a mistake!"
        )

    return out_dict


def get_keys_of_tail(
    data: T_index_to_val, tail: Tail, percentile: float
...
def get_top_n_elements(
    data: T_index_to_val, tail: Tail, n_elems: int
) -> typing.Set[design.PolarIndex]:
...
class StackWithQuickMembership(list):
    ...
    def _add_key(self, value):
        ...
        if i1 in self._contents:
            raise ValueError(
                f"Tried to add {value} but the key, {i1}, was already in here."
            )
...
def get_top_n_elements_maintaining_edge_connectivity(
    ...
    tail: Tail,
    n_elems: int,
    elems_to_patch_buddies_idx: typing.Dict[
        design.PolarIndex, typing.Set[design.PolarIndex]
    ],
    ...
    graph_element_pool_all = {
        elemIdx: element_bucket.Element(
            num=elemIdx,
            conn=design.get_single_element_connection(stent_params, elemIdx),
        )
        for elemIdx in all_in_elements
    }
    graph_all_in = element_bucket.build_graph(
        element_bucket.GraphEdgeEntity.fe_elem_edge, graph_element_pool_all.values()
    )

    sorted_data: StackWithQuickMembership = _evolve_decider_sorted_data(
        data, tail, n_elems
    )
    ...
    def element_pool_is_connected(elem_working_set: typing.Set[design.PolarIndex]):
        ...
        graph = element_bucket.build_graph(
            element_bucket.GraphEdgeEntity.fe_elem_edge, graph_element_pool
        )
        num_components = networkx.number_connected_components(graph)

        return num_components == 1

    def element_pool_has_boundaries_attached(
        elem_working_set: typing.Set[design.PolarIndex],
    ):
        ...
        node_working_set = {
            ...
            for node_idx in design.get_2d_plate_connection_polar_index(elem_idx)
        }
        ...
        for node_idx in sorted(node_working_set):
            ...
            for bc in bcs_not_yet_hit:
                contains_idx, _ = bc.contains_polar_index_and_ratio(
                    stent_params, node_idx
                )
                if contains_idx:
        ...
        for elem_idx in elem_working_set:
            ...
            bottom_bound_cutoff = stent_params.length * (
                0.5 - 0.5 * stent_params.end_connection_length_ratio
            )
            top_bound_cutoff = stent_params.length * (
                0.5 + 0.5 * stent_params.end_connection_length_ratio
            )
            is_in_middle = (
                bottom_bound_cutoff
                <= elem_idx.Z * stent_params.single_element_z_span
                <= top_bound_cutoff
            )
            if is_in_middle:
                if elem_idx.Th == 0:
        ...
        has_all_thetas = (
            len(elems_at_each_slice) + 1 >= stent_params.get_divs_with_sym().Th
        )
    ...
    while num_changed_elements() < n_elems and len(sorted_data) > halfway:
        ...
        if optim_params.one_elem_per_patch and not elem_idx_is_clean:
            print(
                f"  [Conn] {elemIdxCandidate} = {obj_fun_val} skipping... invalidated by a prior patch buddy being changed."
            )
            continue
        ...
        connected_to_candidate = list(
            graph_all_in.neighbors(graph_element_pool_all[elemIdxCandidate])
        )
        if element_pool_is_OK(trial_working_set):
            ...
            if tail.action_is_adding_element:
                suffix_text = (
                    " (but was already in mesh)"
                    if elemIdxCandidate in active_elems_working_set
                    else ""
                )
                ...
                suffix_text = (
                    " (but was not in the mesh???)"
                    if elemIdxCandidate not in active_elems_working_set
                    else ""
                )
            ...
            print(
                f"  [Conn] {elemIdxCandidate} = {obj_fun_val} OK to {DEBUG_verb}{suffix_text}"
            )
            ...
            for reactivated_idx, reactivated_value in reversed(
                _evolve_decider_sorted_data(
                    back_on_stack_elems, tail, len(back_on_stack_elems)
                )
            ):
                print(reactivated_idx)
                if sorted_data.contains_item0(reactivated_idx):
                    print(
                        f"  [Conn] would reactivate {reactivated_idx}={reactivated_value} but it was already in the queue."
                    )
                    ...
                    print(
                        f"  [Conn] reactivating {reactivated_idx}={reactivated_value}."
                    )
                    sorted_data.append((reactivated_idx, reactivated_value))

        else:
            print(
                f"  [Conn] {elemIdxCandidate} = {obj_fun_val} not going to  {DEBUG_verb} {elemIdxCandidate} - would create disconnection."
            )
            for element_i_am_attached_to in connected_to_candidate:
                print(
                    f"  [Conn] stacking {elemIdxCandidate} to be reactivated if {element_i_am_attached_to} changes state."
                )
                back_on_stack_after_change[element_i_am_attached_to.num][
                    elemIdxCandidate
                ] = obj_fun_val
...
class CandidatesFor(typing.NamedTuple):
    ...
    existing_next_round: typing.FrozenSet[
        design.PolarIndex
    ]  # Allowable design space for the next round (current elems and new ones)
    new_introduction: typing.FrozenSet[
        design.PolarIndex
    ]  # Elements we could add but which weren't there last round.
    removal_from_everywhere: typing.FrozenSet[
        design.PolarIndex
    ]  # If we don't limit ourselves to the boundary, these elements can go as well.
...
def get_candidate_elements(
    ...
    fully_populated_indices_including_inadmissable = _get_all_elements(
        design_n_min_1.stent_params
    )

    def get_adj_elements(last_iter_elems, threshold: int):
        # Prepare the set of nodes to which eligible elements are attached.
        connections = (
            design.get_single_element_connection(design_n_min_1.stent_params, elemIdx)
            for elemIdx in last_iter_elems
        )
        ...
        if some_kind_of_limits_here:
            fully_encased_node_connection = (
                2 if design_n_min_1.stent_params.stent_element_dimensions == 2 else 4
            )
            #  boundary_nodes = design.generate_stent_boundary_nodes(design_n_min_1.stent_params)
            boundary_nodes = set()
            pretend_nodes_werent_there = {
                iNodeIdx
                for iNodeIdx, n_attached_elems in nodes_on_last.items()
                if iNodeIdx in boundary_nodes
                and n_attached_elems == fully_encased_node_connection
            }
        ...
        for elemIdx in fully_populated_indices:
            this_elem_nodes = design.get_single_element_connection(
                design_n_min_1.stent_params, elemIdx
            )
            node_with_shared_interface = [
                iNode for iNode in this_elem_nodes if iNode in nodes_on_last
            ]
    ...
    indicies_holes = (
        fully_populated_indices_including_inadmissable - design_n_min_1.active_elements
    )

    # Have to hackily add in the elements on the boundaries to the "removable" set...
    boundary_nodes_idxs = list(
        design.generate_stent_boundary_nodes(design_n_min_1.stent_params)
    )
    boundary_nodes_nums = {
        design.node_from_index(
            design_n_min_1.stent_params.divs, node_idx.R, node_idx.Th, node_idx.Z
        )
        for node_idx in boundary_nodes_idxs
    }
    ...
    removal_partial = get_adj_elements(
        indicies_holes, optim_params.nodes_shared_with_old_design_to_contract
    )
    ...
    existing_next_round = get_adj_elements(
        design_n_min_1.active_elements,
        optim_params.nodes_shared_with_old_design_to_expand,
    )
...
def evolve_decider(
    ...
    design_n_min_1,
    sensitivity_result,
    run_one_args_completed: RunOneArgs,
) -> typing.Set[design.PolarIndex]:
    ...
    max_new_num_unconstrained = int(
        optim_params.max_change_in_vol_ratio
        * design_n_min_1.stent_params.divs.fully_populated_elem_count()
    )
    max_new_num = min(
        max_new_num_unconstrained, max_new_num_unconstrained + delta_n_elems
    )
    if max_new_num < 0:
        max_new_num = 0

    # Keep track of the element indices which haven't been touched by another addition or removal.
    clean_elems_to_patch_buddies_num = (
        run_one_args_completed.get_real_model_patch_to_elem()
    )
    elem_num_to_idx = {
        elem_id: elem_idx
        for elem_idx, elem_id, _ in design.generate_plate_elements_all(
            design_n_min_1.stent_params.divs,
            design_n_min_1.stent_params.stent_element_type,
        )
    }
    ...
    for (
        clean_elem_num,
        other_clean_elem_nums,
    ) in clean_elems_to_patch_buddies_num.items():
        clean_idxs = [
            elem_num_to_idx[other_elem_num] for other_elem_num in other_clean_elem_nums
        ]
    ...
    dirty_elems = set()
    (
        top_new_potential_elems_all,
        dirty_elems,
    ) = get_top_n_elements_maintaining_edge_connectivity(
        optim_params,
        design_n_min_1.stent_params,
        design_n_min_1.active_elements,
        sensitivity_result,
        Tail.top,
        max_new_num,
        clean_elems_to_patch_buddies_idx,
        dirty_elems,
    )
    top_new_potential_elems = {
        idx
        for idx in top_new_potential_elems_all
        if idx in candidates_for.existing_next_round
    }
    ...
    existing_elems_only_ranked = {
        idx: val
        for idx, val in sensitivity_result.items()
        if idx in design_n_min_1.active_elements and idx in candidates_for_removal
    }
    ...
    to_go, _ = get_top_n_elements_maintaining_edge_connectivity(
        optim_params,
        design_n_min_1.stent_params,
        elem_working_set,
        existing_elems_only_ranked,
    ...
    new_active_elems = (
        design_n_min_1.active_elements | top_new_potential_elems
    ) - to_go
    return new_active_elems


def _hist_status_stage_from_rank_comp(
    rank_comp: score.T_AnyRankingComponent,
) -> history.StatusCheckStage:
...
class RankingResults(typing.NamedTuple):
    ...
    final_ranking_component_to_unsmoothed: typing.Dict[
        common.GlobalStatusType, typing.Tuple[str, T_index_to_val]
    ]
    pos_rows: typing.List[db_defs.NodePos]

    def get_ordered_ranking_with_filter(
        self, max_to_filter_out: int
    ) -> T_index_to_two_val:
        ...
        first_ordering = itertools.chain(
            itertools.repeat(FILTERED_OUT, max_to_filter_out),
            itertools.repeat(LEFT_ALONE),
        )
        filter_out_priority_sorted = reversed(
            sorted((val, key) for key, val in self.filter_out_priority.items())
        )
        for should_filter, (filter_val, elem_idx) in zip(
            first_ordering, filter_out_priority_sorted
        ):
            overall_ranking[elem_idx] = (
                should_filter,
                self.final_ranking_component_smoothed.get(elem_idx, 0.0),
            )
...
def _combine_constraint_violations_rows(
    elem_num_to_indices, cons_viol_rows: typing.Iterable[score.FilterRankingComponent]
) -> T_index_to_val:
...
T_filter_applicable_primary_components = typing.Union[
    db_defs.ElementFatigueResult, db_defs.ElementPEEQ
]


def _get_raw_data_relevant_to_constraint_filter(
    data: datastore.Datastore, one_frame, maybe_only_these_elem_nums
) -> typing.Iterable[T_filter_applicable_primary_components]:
    for elem_component in T_filter_applicable_primary_components.__args__:
        yield from data.get_all_rows_at_frame(
            elem_component, one_frame, only_these_elem_nums=maybe_only_these_elem_nums
        )
...
def _get_ranking_functions(
    ...
    if optim_params.region_gradient:
        ...
        first_grad_track = max(
            0, final_grad_track - optim_params.region_gradient.n_past_increments
        )
        grad_track_steps = range(first_grad_track, final_grad_track)

        recent_gradient_input_data = get_gradient_input_data(
            optim_params, grad_track_steps
        )
        vicinity_ranking = list(
            score.get_primary_ranking_local_region_gradient(
                design_n_min_1.stent_params, recent_gradient_input_data, statistics.mean
            )
        )
    ...
    elem_components_data = list(
        optim_params.get_all_elem_components(
            patch_model_context=model_info.is_sub_model
        )
    )
    for include_in_opt_comp, elem_component in elem_components_data:
        for (
            include_in_opt_filter,
            one_filter,
        ) in optim_params.get_all_primary_ranking_fitness_filters():
            include_in_opt = include_in_opt_comp and include_in_opt_filter

            elem_results_this_submod = list(
                data.get_all_rows_at_frame(
                    elem_component,
                    one_frame,
                    only_these_elem_nums=maybe_only_these_elem_nums,
                )
            )
            this_comp_rows = score.get_primary_ranking_components(
                include_in_opt, one_filter, optim_params, elem_results_this_submod
            )
    ...
    pos_rows_submod = list(
        data.get_all_rows_at_frame(
            db_defs.NodePos, one_frame, only_these_node_nums=maybe_only_these_node_nums
        )
    )
    pos_rows = [
        pos_row._replace(node_num=model_info.model_to_real_node(pos_row.node_num))
        for pos_row in pos_rows_submod
    ]
    for include_in_opt, one_func in optim_params.get_all_node_position_components(
        patch_model_context=model_info.is_sub_model
    ):
        this_comp_rows = one_func(
            optim_params, include_in_opt, design_n_min_1, pos_rows
        )
        raw_elem_rows.append(list(this_comp_rows))
    ...
    for one_submodel_id_prim_result in raw_elem_rows:
        ...
        if one_submodel_id_prim_result:
            with_main_model_ids = [
                elem_row._replace(
                    elem_id=model_info.model_to_real_elem(elem_row.elem_id)
                )
                for elem_row in one_submodel_id_prim_result
            ]
    ...
    elem_num_to_indices = {
        iElem: idx
        for iElem, idx in design.generate_elem_indices(design_n_min_1.stent_params.divs)
    }
    elem_indices_to_num = {
        idx: iElem
        for iElem, idx in design.generate_elem_indices(design_n_min_1.stent_params.divs)
    ...
    for (
        global_status_type,
        f_elem_result,
    ) in optim_params.get_multi_level_aggregator().items():

        def is_matching_func_output(one_rank):
            return all(
                prc.comp_name.startswith(f_elem_result.__doc__)
                and isinstance(prc, score.PrimaryRankingComponent)
                for prc in one_rank
            )

        this_level_ranks = [
            one_rank for one_rank in all_ranks if is_matching_func_output(one_rank)
        ]

        if len(this_level_ranks) != 1:
            raise ValueError("???")

        multi_level_agg[global_status_type] = f_elem_result.__doc__, {
            elem_num_to_indices[prc.elem_id]: prc.value for prc in this_level_ranks[0]
        }
    ...
    unsmoothed = {
        elem_num_to_indices[iElem]: val for iElem, val in overall_rank.items()
    }
    smoothed = gaussian_smooth(optim_params, design_n_min_1.stent_params, unsmoothed)

    def append_additional_output(
        idx_to_val: typing.Dict[design.PolarIndex, float], comp_name: str
    ):
        additional_ranking = [
    ...
    aba_db_rows_submod_ids = list(
        _get_raw_data_relevant_to_constraint_filter(
            data, one_frame, maybe_only_these_elem_nums
        )
    )
    aba_db_rows_full_mod_ids = [
        elem_row._replace(elem_num=model_info.model_to_real_elem(elem_row.elem_num))
        for elem_row in aba_db_rows_submod_ids
    ]
    for (
        include_in_opt_comp,
        one_filter_func,
    ) in optim_params.get_all_filter_components():
        this_filter_rows = list(
            one_filter_func(include_in_opt_comp, aba_db_rows_full_mod_ids)
        )
        constraint_violations.extend(this_filter_rows)
        if this_filter_rows:
            all_ranks.append(this_filter_rows)

    filter_out_priority = _combine_constraint_violations_rows(
        elem_num_to_indices, constraint_violations
    )
...
def prepare_patch_models(
    working_dir: pathlib.Path,
    optim_params: optimisation_parameters.OptimParams,
    iter_prev: int,
) -> typing.Iterable[T_PatchModelPair]:
    ...
    node_num_to_pos = {
        iNode: xyz for iNode, _, xyz in design.generate_nodes(stent_params)
    }

    elem_num_to_indices = {
        elem_num: polar_index
        for elem_num, polar_index in design.generate_elem_indices(stent_params.divs)
    }
    elem_indices_to_num = {
        polar_index: elem_num for elem_num, polar_index in elem_num_to_indices.items()
    }

    all_node_polar_index_admissible = (
        stent_params.get_all_node_polar_indices_admissible()
    )
    ...
    with history.History(history_db) as hist:
        ...
        design_n_min_1 = design.StentDesign(
            ...
            active_elements=frozenset(
                (
                    elem_num_to_indices[iElem]
                    for iElem in snapshot_n_min_1.active_elements
                )
            ),
    ...
    graph_fe_elems = {
        elem_num: element_bucket.Element(
            elem_num, design.get_single_element_connection(stent_params, polar_index)
        )
    ...
    anchor_nodes = sum(
        len(data)
        for data in design_n_min_1.get_boundary_conds_to_node_to_stiffness().values()
    )
    need_space_for_nodes = (
        stent_params.divs.fully_populated_node_count() + anchor_nodes + 10
    )
    delta_offset = 10 ** (len(str(need_space_for_nodes)))
    offset_counter = itertools.count(delta_offset, delta_offset)

    graph = element_bucket.build_graph(
        element_bucket.GraphEdgeEntity.node, graph_fe_elems.values()
    )

    with patch_manager.PatchManager(node_num_to_pos) as this_patch_manager:
        with datastore.Datastore(db_fn_prev) as data:
            this_patch_manager.ingest_node_pos(
                data.get_all_frames(), data.get_all_rows(db_defs.NodePos)
            )
        ...
        for initial_active_state, elem_idxs in init_state_and_elems:
            for polar_index in sorted(elem_idxs):
                ...
                design_n_min_1_with_elem = design_n_min_1.with_additional_elements(
                    [polar_index]
                )
                ...
                (
                    boundary_node_nums,
                    interior_fe_elems,
                ) = element_bucket.get_fe_nodes_on_boundary_interface(
                    graph, optim_params.patch_hops, graph_elem
                )
                elem_nums = frozenset(
                    fe_elem.num
                    for fe_elem in interior_fe_elems
                    if elem_num_to_indices[fe_elem.num]
                    in design_n_min_1_with_elem.active_elements
                )
...
def produce_patch_models(
    working_dir: pathlib.Path, iter_prev: int
) -> typing.Dict[str, typing.List[patch_manager.SubModelInfoBase]]:
    ...
    sub_model_infos_all = list(
        prepare_patch_models(working_dir, optim_params, iter_prev)
    )
    ...
    if optim_params.filter_singular_patches:
        if computer.this_computer.n_processes_unlicensed > 1:
            with multiprocessing.Pool(
                processes=computer.this_computer.n_processes_unlicensed
            ) as pool:
                sub_model_infos = [
                    smi_pair
                    for is_ok, smi_pair in pool.imap_unordered(
                        _smi_pair_are_good, sub_model_infos_all
                    )
                    if is_ok
                ]
    ...
    print(
        f"Time to check singularity of {len(sub_model_infos_all)} patch pairs: {time.time() - t_before_patch} sec"
    )

    # Batch them up into even-ish chunks
    chunk_size_ideal = (
        len(sub_model_infos) / computer.this_computer.n_abaqus_parallel_solves
    )
    ...
    FLOOR_ELEMS_IN_MODEL = (
        1000
        if optim_params.analysis_step_type == stent_opt.abaqus_model.step.StepStatic
        else 250
    )
    floor_chunk_size = math.ceil(
        FLOOR_ELEMS_IN_MODEL / optim_params.nominal_number_of_patch_elements
    )
    chunk_size = max(
        floor_chunk_size, chunk_size_round
    )  # Don't need to make it too crazy tiny...
    print(
        f"  Chunk Size [Ideal / Round / Used]: {chunk_size_ideal} / {chunk_size_round} / {chunk_size}"
    )
    #  chunk_size = 1 # TEMP!!!
    ...
    for suffix, sub_model_info_list in get_prefix_and_smi():
        ...
        if sub_model_info_list:
            ...
            flat_sub_model_infos = [
                sub_mod_inf
                for sub_model_info_pair in sub_model_info_list
                for sub_mod_inf in sub_model_info_pair
            ]
            ...
            stent_design_with_extras = flat_sub_model_infos[
                0
            ].stent_design.with_additional_elements(
                fsmi.reference_elem_idx for fsmi in flat_sub_model_infos
            )
            construct_model.make_stent_model(
                optim_params, stent_design_with_extras, flat_sub_model_infos, sub_fn_inp
            )
...
def _get_combined_global_objective_function(ranking_results: RankingResults):
    ...
    for comp_agg_func, (
        func_doc,
        elem_idx_to_val,
    ) in ranking_results.final_ranking_component_to_unsmoothed.items():
...
def _get_change_in_overall_objective_from_patches(
    ...
    def make_working_data():
        for model_info, ranking_results in model_info_to_patch_rank.items():
            ...
            overall_term_name, obj_func_val = _get_combined_global_objective_function(
                ranking_results
            )
    ...
    for elem_num, patch_rows in itertools.groupby(working_data, get_elem_num):
        activation_data = {
            active_state: obj_func for _, active_state, obj_func in patch_rows
        }
...
def process_completed_simulation(
    run_one_args: RunOneArgs,
) -> typing.Tuple[
    design.StentDesign, typing.Dict[patch_manager.SubModelInfoBase, RankingResults]
]:
    ...
    db_fn_prev = history.make_fn_in_dir(
        run_one_args.working_dir, ".db", iter_prev, run_one_args.patch_suffix
    )
    ...
    with datastore.Datastore(db_fn_prev) as data:
        ...
        one_frame = data.get_maybe_last_frame_of_instance("STENT-1")
        abaqus_created_primary_rows = list(
            data.get_all_rows_at_all_frames_any_element_type()
        )
        try:
            composite_primary_rows_one = score.compute_composite_ranking_component_one(
                optim_params, abaqus_created_primary_rows
            )
            composite_primary_rows_two = score.compute_composite_ranking_component_two(
                optim_params, abaqus_created_primary_rows
            )
    ...
    elem_num_to_indices = {
        iElem: idx for iElem, idx in design.generate_elem_indices(stent_params.divs)
    }
    ...
    for run_one_args_patch in run_one_args.child_patch_run_one_args:
        ...
        _, this_model_info_to_patch_rank = process_completed_simulation(
            run_one_args_patch
        )
        model_info_to_patch_rank.update(this_model_info_to_patch_rank)

    elem_patch_deltas = _get_change_in_overall_objective_from_patches(
        model_info_to_patch_rank
    )
    ...
    with datastore.Datastore(db_fn_prev) as data:
        ...
        in_db_form = [
            db_defs.ElementGlobalPatchSensitivity(
                frame_rowid=None,
                elem_num=elem_num,
                gradient_from_patch=gradient_from_patch,
            )
    ...
    with datastore.Datastore(db_fn_prev) as data:
        ...
        for model_info in run_one_args.model_infos:
            ranking_result = _get_ranking_functions(
                run_one_args, model_info, iter_prev, design_n_min_1, data
            )
            ...
            if run_one_args.is_full_model:
                ...
                _log_completed_in_history_db(
                    history_db,
                    ranking_result,
                    global_status_raw,
                    applied_load_results,
                    optim_params,
                    design_n_min_1,
                    iter_prev,
                )
...
def _get_sum_of_reaction_forces(
    data: datastore.Datastore, iteration_num
) -> typing.Iterable[history.GlobalStatus]:
    ...
    def get_delta_components():
        ...
        good_rows = [
            row for row in rows if row.frame_rowid in {last_rowid, second_last_rowid}
        ]
    ...
    for global_name, global_val in (
        ...
        yield history.GlobalStatus(
            iteration_num,
            history.GlobalStatusType.abaqus_history_result,
            global_name,
            global_val,
        )


def _log_completed_in_history_db(
    history_db,
    ranking_result,
    global_status_raw,
    applied_load_results,
    ...
    pos_lookup = {
        row.node_num: base.XYZ(x=row.X, y=row.Y, z=row.Z)
        for row in ranking_result.pos_rows
    }
    ...
    with history.History(history_db) as hist:
        ...
        status_checks = (
            ...
            for rank_comp_list in ranking_result.all_ranks
            for rank_comp in rank_comp_list
        )
        ...
        node_pos_for_hist = (
            history.NodePosition(
                iteration_num=iter_prev, node_num=node_num, x=pos.x, y=pos.y, z=pos.z
            )
            for node_num, pos in pos_lookup.items()
        )
        ...
        vol_ratios = [
            history.GlobalStatus(
                ...
                global_status_value=optim_params.volume_target_func(
                    optim_params.volume_target_opts, iter_prev
                ),
            ),
            history.GlobalStatus(
                iteration_num=iter_prev,
...
T_ProdNewGen = typing.Callable[
    [pathlib.Path, design.StentDesign, RankingResults, int, str], design.StentDesign
]
...
def produce_new_generation(
    ...
    delta_n_elems = int(
        optim_params.max_change_in_vol_ratio
        * design_prev.stent_params.divs.fully_populated_elem_count()
    )
    sensitivity_ranking = ranking_result.get_ordered_ranking_with_filter(delta_n_elems)

    new_active_elems = evolve_decider(
        optim_params, design_prev, sensitivity_ranking, run_one_args_completed
    )
...
def make_plot_tests(working_dir: pathlib.Path, iter_n: int):
    ...
    recent_gradient_input_data = list(
        get_gradient_input_data(optim_params, history_iters)
    )

    vicinity_ranking = list(
        score.get_primary_ranking_local_region_gradient(
            stent_params, recent_gradient_input_data, statistics.mean
        )
    )
    ...
    for one_iter in history_iters:
        ...
        with datastore.Datastore(db_fn) as data:
            ...
            stress_rows = list(
                data.get_all_rows_at_frame(db_defs.ElementStress, one_frame)
            )
            pos_rows = list(data.get_all_rows_at_frame(db_defs.NodePos, one_frame))

        pos_lookup = {
            row.node_num: base.XYZ(x=row.X, y=row.Y, z=row.Z) for row in pos_rows
        }
...
def _make_testing_run_one_args(
    working_dir, stent_design: design.StentDesign, optim_params=None, iter_this=0
) -> RunOneArgs:

    from stent_opt.make_stent import (
        run_model,
        working_dir_extract,
        make_full_model_list,
        process_pool_run_and_process,
    )
...
def evolve_decider_test():
    ...
    run_one_args_input = _make_testing_run_one_args(
        pathlib.Path(r"C:\Simulations\StentOpt\AA-344"),
        design_n_min_1,
        optim_params,
        iter_n_min_1,
    )
    ...
    with history.History(history_db) as hist:
        ...
        smoothed_checks = [
            st for st in status_checks if st.stage == history.StatusCheckStage.smoothed
        ]
        smoothed = {
            elem_num_to_indices[st.elem_num]: st.metric_val for st in smoothed_checks
        }

    new_active_elems = evolve_decider(
        optim_params, design_n_min_1, smoothed, run_one_args_completed
    )
...
def run_test_process_completed_simulation():
    ...
    testing_run_one_args_skeleton = _make_testing_run_one_args(
        working_dir, old_design, iter_this=17
    )

    testing_run_one_args_completed = process_pool_run_and_process(
        testing_run_one_args_skeleton
    )
    one_design, model_info_to_rank = process_completed_simulation(
        testing_run_one_args_completed
    )
    ...
    one_new_design = produce_new_generation(
        working_dir, one_design, one_ranking, testing_run_one_args_completed, "TESTING"
    )
...
def spot_check_smoothing():
    ...
    with history.History(fn) as hist:
        ...
        res_unsmoothed = [
            sc for sc in res_all if sc.stage == history.StatusCheckStage.secondary
        ]
        ...
        if len(res_check_names) != 1:
            raise ValueError(
                f"Meant to get one single metric for this check - got {res_check_names}"
            )
...
if False:
    ...
    new_design = produce_new_generation(
        working_dir,
        one_design,
        one_ranking,
        iter_this,
        "generation.produce_new_generation",
    )
    # print(new_design)
