...
def _get_rigid_motion_sorkine_hornung(
    p_orig: numpy.array, q_deformed: numpy.array
) -> RigidMotion:
...
def _get_rigid_motion_horn_1987(
    p_orig: numpy.array, q_deformed: numpy.array
) -> RigidMotion:
    """
...
def nodal_deformation(
    nominal_length: float, p_orig: numpy.array, q_deformed: numpy.array
) -> float:
...
def test_from_real_data_should_be_small():
    orig = {
        2850: XYZ(x=0.30515651581082903, y=2.7085427135678395, z=0.5739159353583025),
    ...
    deformed = {
        2850: XYZ(x=1.3744884133338928, y=2.9893102645874023, z=2.434308886528015),
...
def _euler_angle_rotation_matrix(
...
def nodal_deformation_least_squares(
    nominal_length: float, orig: T_NodeMap, deformed: T_NodeMap
) -> float:
    """Gets some normalised deformation measure for the subset of nodes overall."""
    ...
    def f_to_minimise(x):
        ...
        rigid_motion = RigidMotion(
            R=_euler_angle_rotation_matrix(x[0], x[1], x[2]), t=x[3:6]
        )
