...
T_AnyRankingComponent = typing.Union[
    PrimaryRankingComponent, SecondaryRankingComponent, FilterRankingComponent
]
T_ListOfComponentLists = typing.List[
    typing.List[T_AnyRankingComponent]
]  # A list of lists, where the sub lists are all primary or all secondary
...
class CompositeResultHelper:
    ...
    def __init__(self, ret_obj):
        ...
        self.elem_num_to_type_to_all_rows = collections.defaultdict(
            default_dict_list_maker
        )
    ...
    def ingest_rows(self, nt_rows_all_from_frame):
        for row in nt_rows_all_from_frame:
            ...
            self.elem_num_to_type_to_all_rows[row.elem_num][row_type].append(
                row.get_last_value()
            )
    ...
    def get_last_point(
        self, elem_num, db_row_type: typing.Type[optimisation_parameters.T_elem_result]
    ) -> float:
...
T_Any_Composite_Res = typing.Union[
    db_defs.ElementCustomCompositeOne, db_defs.ElementCustomCompositeTwo
]


def primary_composite_stress_peeq_energy(
    composite_result_helper: CompositeResultHelper,
) -> typing.Iterable[T_Any_Composite_Res]:
    ...
    for elem_num in composite_result_helper.get_all_element_nums():
        ...
        ElasticEnergy = composite_result_helper.get_last_point(
            elem_num, db_defs.ElementEnergyElastic
        )

        elast_energy_min, elast_enery_max = composite_result_helper.row_type_to_range[
...
def primary_composite_stress_over_crit(
    composite_result_helper: CompositeResultHelper,
) -> typing.Iterable[T_Any_Composite_Res]:
...
def primary_composite_stress_peeq_energy_neg(
    composite_result_helper: CompositeResultHelper,
) -> typing.Iterable[T_Any_Composite_Res]:
    ...
    for elem_num in composite_result_helper.get_all_element_nums():
        ...
        one_val = (
            -1 * (0.1 + vM) * (0.01 + PEEQ) * (elast_enery_max - ElasticEnergy + 0.01)
        )
        yield composite_result_helper.ret_obj(
            frame_rowid=None,
            elem_num=elem_num,
...
def primary_composite_stress_peeq_energy_factor_test(
    composite_result_helper: CompositeResultHelper,
) -> typing.Iterable[T_Any_Composite_Res]:
    ...
    for elem_num in composite_result_helper.get_all_element_nums():
        ...
        one_val = (
            (x * high_vM + vM)
            * (x * high_PEEQ + PEEQ)
            * (elast_enery_max - ElasticEnergy + x * high_elastic_energy)
        )
        yield composite_result_helper.ret_obj(
            frame_rowid=None,
            elem_num=elem_num,
...
def primary_composite_stress_peeq_energy_factor_test_v2(
    composite_result_helper: CompositeResultHelper,
) -> typing.Iterable[T_Any_Composite_Res]:
    ...
    for elem_num in composite_result_helper.get_all_element_nums():
        ...
        one_val = ((x * high_vM + vM) * (x * high_PEEQ + PEEQ)) * (
            elast_enery_max - ElasticEnergy + x * high_elastic_energy
        )
        yield composite_result_helper.ret_obj(
            frame_rowid=None,
...
def primary_composite_stress_peeq_energy_factor_test_v3(
    composite_result_helper: CompositeResultHelper,
) -> typing.Iterable[T_Any_Composite_Res]:
    ...
    for elem_num in composite_result_helper.get_all_element_nums():
        ...
        one_val = (
            vM / high_vM
            + PEEQ / high_PEEQ
            + (elast_enery_max - ElasticEnergy) / high_elastic_energy
        )
        yield composite_result_helper.ret_obj(
            frame_rowid=None,
            elem_num=elem_num,
...
def primary_composite_stress_peeq_energy_factor_test_v4(
    composite_result_helper: CompositeResultHelper,
) -> typing.Iterable[T_Any_Composite_Res]:
...
def primary_composite_stress_peeq_energy_factor_test_v4_neg(
    composite_result_helper: CompositeResultHelper,
) -> typing.Iterable[T_Any_Composite_Res]:
    ...
    for elem_num in composite_result_helper.get_all_element_nums():
        ...
        one_val = -1 * (
            vM / high_vM + PEEQ / high_PEEQ - ElasticEnergy / high_elastic_energy
        )
        yield composite_result_helper.ret_obj(
            frame_rowid=None,
            elem_num=elem_num,
...
def primary_composite_stress_and_peeq(
    composite_result_helper: CompositeResultHelper,
) -> typing.Iterable[T_Any_Composite_Res]:
...
def primary_composite_stress_and_peeq_and_load(
    composite_result_helper: CompositeResultHelper,
) -> typing.Iterable[T_Any_Composite_Res]:
    ...
    for elem_num in composite_result_helper.get_all_element_nums():
        vM = composite_result_helper.get_last_point(elem_num, db_defs.ElementStress)
        PEEQ = composite_result_helper.get_last_point(elem_num, db_defs.ElementPEEQ)
        NForce = composite_result_helper.get_last_point(
            elem_num, db_defs.ElementNodeForces
        )
...
def primary_composite_stress_and_peeq_and_load_inv(
    composite_result_helper: CompositeResultHelper,
) -> typing.Iterable[T_Any_Composite_Res]:
    ...
    for elem_num in composite_result_helper.get_all_element_nums():
        ...
        one_val = (
            vM / high_vM + PEEQ / high_PEEQ + high_NodeForce / (NForce + low_NodeForce)
        )
        yield composite_result_helper.ret_obj(
            frame_rowid=None,
            elem_num=elem_num,
...
def primary_composite_stress_and_peeq_and_load_inv_log(
    composite_result_helper: CompositeResultHelper,
) -> typing.Iterable[T_Any_Composite_Res]:
    ...
    for elem_num in composite_result_helper.get_all_element_nums():
        ...
        one_val = (
            vM / high_vM
            + PEEQ / high_PEEQ
            + max(0.0, math.log(high_NodeForce / (NForce + low_NodeForce)))
        )
        yield composite_result_helper.ret_obj(
            frame_rowid=None,
            elem_num=elem_num,
...
def primary_composite_energy_final_gradient(
    composite_result_helper: CompositeResultHelper,
) -> typing.Iterable[T_Any_Composite_Res]:
    """delta(EnergyTotal) over last two result frames"""

    for elem_num in composite_result_helper.get_all_element_nums():
        ElementEnergyElastic = composite_result_helper.get_all_points(
            elem_num, db_defs.ElementEnergyElastic
        )
        ElementEnergyPlastic = composite_result_helper.get_all_points(
            elem_num, db_defs.ElementEnergyPlastic
        )
...
def primary_composite_energy_final_gradient_neg(
    composite_result_helper: CompositeResultHelper,
) -> typing.Iterable[T_Any_Composite_Res]:
    """-delta(EnergyTotal) over last two result frames"""
...
def primary_composite_energy(
    composite_result_helper: CompositeResultHelper,
) -> typing.Iterable[T_Any_Composite_Res]:
    """EnergyTotal"""

    for elem_num in composite_result_helper.get_all_element_nums():
        ElementEnergyElastic = composite_result_helper.get_last_point(
            elem_num, db_defs.ElementEnergyElastic
        )
        ElementEnergyPlastic = composite_result_helper.get_last_point(
            elem_num, db_defs.ElementEnergyPlastic
        )
...
def primary_composite_energy_neg(
    composite_result_helper: CompositeResultHelper,
) -> typing.Iterable[T_Any_Composite_Res]:
    """- EnergyTotal"""

    # TODO - make this normalised and energy density
...
def compute_composite_ranking_component_one(
    optim_params: optimisation_parameters.OptimParams, nt_rows_all_from_frame
) -> typing.Iterable[db_defs.ElementCustomCompositeOne]:
...
def compute_composite_ranking_component_two(
    optim_params: optimisation_parameters.OptimParams, nt_rows_all_from_frame
) -> typing.Iterable[db_defs.ElementCustomCompositeTwo]:
    """This computes a composite function based on existing results in the Datastore. Called after Abaqus has populated it."""
...
def _get_primary_ranking_components_raw(
    include_in_opt, optim_params: optimisation_parameters.OptimParams, nt_rows
...
def _get_primary_ranking_deviation(
    include_in_opt: bool,
    optim_params: optimisation_parameters.OptimParams,
    central_value_producer,
    nt_rows,
) -> typing.Iterable[PrimaryRankingComponent]:
    """Gets a ranking component which makes elements close to the mean the best."""

    raw_primary_res = list(
        _get_primary_ranking_components_raw(include_in_opt, optim_params, nt_rows)
    )
    ...
    deviations = [
        prc._replace(value=central_value - prc.value) for prc in raw_primary_res
    ]
...
def get_primary_ranking_components(
    include_in_opt: bool,
    one_filter: common.PrimaryRankingComponentFitnessFilter,
    optim_params: optimisation_parameters.OptimParams,
    nt_rows,
) -> typing.Iterable[PrimaryRankingComponent]:
    ...
    if one_filter.is_deviation_from_central_value:
        nt_producer = _get_primary_ranking_deviation(
            include_in_opt,
            optim_params,
            one_filter.get_central_value_function(),
            nt_rows,
        )

    else:
        nt_producer = _get_primary_ranking_components_raw(
            include_in_opt, optim_params, nt_rows
        )

    # Add on the suffix for the filter.
    with_filter_suffix = (
        prc._replace(comp_name=f"{prc.comp_name} {one_filter.name}")
        for prc in nt_producer
    )
    yield from with_filter_suffix


def get_primary_ranking_element_distortion(
    optim_params: optimisation_parameters.OptimParams,
    include_in_opt: bool,
    old_design: "design.StentDesign",
    nt_rows_node_pos,
) -> typing.Iterable[PrimaryRankingComponent]:

    elem_indices_to_num = {
        idx: iElem
        for iElem, idx in design.generate_elem_indices(old_design.stent_params.divs)
    }
    ...
    for elem_idx in old_design.active_elements:
        ...
        try:
            elem_connection = design.get_single_element_connection(
                old_design.stent_params, elem_idx
            )
            yield PrimaryRankingComponent(
                comp_name="InternalAngle",
                elem_id=elem_num,
                value=_max_delta_angle_of_element(
                    old_design.stent_params, node_to_pos, elem_connection
                ),
                include_in_opt=include_in_opt,
            )
...
def _transform_patch_over_boundary(stent_design: "design.StentDesign", node_dict_xyz):
    ...
    node_dict_r_th_z = {
        node_key: xyz.to_r_th_z() for node_key, xyz in node_dict_xyz.items()
    }
    ...
    def maybe_rotated(r_th_z: base.RThZ) -> base.RThZ:
        if r_th_z.theta_deg > half_angle:
            return r_th_z._replace(
                theta_deg=r_th_z.theta_deg - stent_design.stent_params.angle
            )
    ...
    rotated_node_dict = {
        node_key: maybe_rotated(r_th_z) for node_key, r_th_z in node_dict_r_th_z.items()
    }
    span_rotated = span(rotated_node_dict)

    if span_rotated < half_angle:
        rotated_in_xyz = {
            node_key: r_th_z.to_xyz() for node_key, r_th_z in rotated_node_dict.items()
        }
        return rotated_in_xyz

    raise ValueError(
        f"Could not find an rotation under {half_angle} degrees - original was {span_orig}, rotated was {span_rotated}."
    )
...
def get_primary_ranking_local_region_gradient(
    ...
    region_reducer: typing.Callable[
        [typing.Iterable[float]], float
    ],  # e.g., max or statistics.mean.
) -> typing.Iterable[PrimaryRankingComponent]:
    ...
    elem_indices_to_num = {
        ...
        for iElem, idx in design.generate_elem_indices(
            recent_gradient_input_data[0].stent_design.stent_params.divs
        )
    }
    ...
    all_active_element_nums_sets = (
        set(one_snapshot.element_ranking_components.keys())
        for one_snapshot in recent_gradient_input_data
    )
    all_active_element_nums = set.union(*all_active_element_nums_sets)
    all_active_element_idxs = {
        elem_num_to_index[iElem] for iElem in all_active_element_nums
    }
    stent_design_all_elems_active = recent_gradient_input_data[0].stent_design._replace(
        active_elements=frozenset(all_active_element_idxs)
    )

    STENCIL_LENGTH = 4 * stent_params.minimum_element_length  # mm
    elem_to_elems_in_range = graph_connection.element_idx_to_elems_within(
        STENCIL_LENGTH, stent_design_all_elems_active
    )
    elem_num_to_region_elem_nums = {
        elem_indices_to_num[elem_idx]: [
            elem_indices_to_num[one_elem_idx] for one_elem_idx in many_elem_idxs
        ]
        for elem_idx, many_elem_idxs in elem_to_elems_in_range.items()
    }

    # Get the maximum or average stress, for each element, within a radius, at each historical snapshot.
    gradient_datapoints = []
    for one_snapshot, (elem_num, region_elem_nums) in itertools.product(
        recent_gradient_input_data, elem_num_to_region_elem_nums.items()
    ):
        raw_values = [
            one_snapshot.element_ranking_components[one_elem_num].value
            for one_elem_num in region_elem_nums
            if one_elem_num in one_snapshot.element_ranking_components
        ]
    ...
    for elem_num, iterable_of_datapoints in itertools.groupby(
        gradient_datapoints, get_elem_num
    ):
        status_vals = []
        response_vals = []
...
def get_primary_ranking_macro_deformation(
    optim_params: optimisation_parameters.OptimParams,
    include_in_opt: bool,
    old_design: "design.StentDesign",
    nt_rows_node_pos,
    ...
    elem_to_nodes_in_range = graph_connection.element_idx_to_nodes_within(
        optim_params.local_deformation_stencil_length, old_design
    )

    idx_num_elem = [
        (idx, iElem, elem)
        for idx, iElem, elem in design.generate_stent_part_elements(
            old_design.stent_params
        )
        if idx in old_design.active_elements
    ]
    elem_indices_to_num = {idx: iElem for (idx, iElem, elem) in idx_num_elem}

    node_to_pos_deformed = {
        row.node_num: base.XYZ(x=row.X, y=row.Y, z=row.Z) for row in nt_rows_node_pos
    }

    node_to_pos_original = {
        iNode: r_th_z.to_xyz()
        for iNode, r_th_z in design.get_node_num_to_pos(
            stent_params=old_design.stent_params
        ).items()
        if iNode in node_to_pos_deformed
    }
    ...
    def prepare_node_patch(node_dict_xyz, patch_node_nums):
        raw = {
            iNode: xyz
            for iNode, xyz in node_dict_xyz.items()
            if iNode in patch_node_nums
        }
        if old_design.stent_params.wrap_around_theta:
    ...
    for elem_idx, node_num_set in elem_to_nodes_in_range.items():
        ...
        node_indices = [
            node_key_to_index[node_key]
            for node_key in node_num_set
            if node_key in node_key_to_index
        ]
        ...
        this_val = deformation_grad.nodal_deformation(
            optim_params.local_deformation_stencil_length, np_orig_sub, np_def_sub
        )
        for node_num in node_num_set:
            node_num_to_contribs[node_num].append(this_val)

    node_num_to_overall_ave = {
        node_num: statistics.mean(data)
        for node_num, data in node_num_to_contribs.items()
    }
    ...
    for elem_idx, elem_num in elem_indices_to_num.items():
        nodal_vals = [
            node_num_to_overall_ave[node_num]
            for node_num in elem_idx_to_nodes[elem_idx]
        ]
        yield PrimaryRankingComponent(
...
def constraint_filter_not_yielded_out(
    include_in_opt, nt_rows
) -> typing.Iterable[FilterRankingComponent]:
    PEEQ_LIMIT = 0.25
...
def constraint_filter_within_fatigue_life(
    include_in_opt, nt_rows
) -> typing.Iterable[FilterRankingComponent]:
    FATIGUE_LIMIT = 1.0
...
def _max_delta_angle_of_element(
    stent_params: "design.StentParams",
    node_to_pos: typing.Dict[int, base.XYZ],
    elem_connection,
) -> float:
    corner_points_numpy = {
    ...
    for one_face_idx in FACES_OF[stent_params.stent_element_type]:
        ...
        for one_corner_idx in CORNERS_OF_QUAD:
            ...
            cosine_angle = numpy.dot(ba, bc) / (
                numpy.linalg.norm(ba) * numpy.linalg.norm(bc)
            )
...
def _removed_observer_only_components(
    list_of_lists: typing.List[typing.List[PrimaryRankingComponent]],
) -> typing.List[typing.List[PrimaryRankingComponent]]:
    out_list_of_lists = []
...
def get_secondary_ranking_scaled_sum(
    list_of_lists: typing.List[typing.List[PrimaryRankingComponent]],
) -> typing.Iterable[SecondaryRankingComponent]:
    ...
    if len(include_in_opt_lists) == 1:
        ...
        for one_list in include_in_opt_lists:
            ...
            scaled_primary = {
                prim_rank.elem_id: (prim_rank.value + offset_value) / scale_factor_safe
                for prim_rank in one_list
            }
...
def OLD_get_secondary_ranking_sum_of_norm(
    list_of_lists: typing.List[typing.List[PrimaryRankingComponent]],
) -> typing.Iterable[SecondaryRankingComponent]:
